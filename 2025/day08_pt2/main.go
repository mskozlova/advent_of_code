...
func main() {
	...
	graph := buildGraph(edges[:N_ITER], len(boxes))

	for edge_id := N_ITER; edge_id < len(edges); edge_id++ {
		edge := edges[edge_id]
		updateGraph(&graph, edge)
		circuits := countCircuits(&graph)
		if len(circuits) == 1 {
			node1 := boxes[edge.id1]
			node2 := boxes[edge.id2]
			fmt.Println(edge)
			fmt.Println(node1.x * node2.x)
			break
		}
		fmt.Printf("Edges: %d\tCircuits: %d\n", edge_id, len(circuits))
	}
}
...
func updateGraph(graph *map[int][]int, e Edge) {
	addEdge(graph, e.id1, e.id2)
	addEdge(graph, e.id2, e.id1)
}
