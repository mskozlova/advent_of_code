...
class Monkey:
    ...
    def __throw(self, item, monkey_lcm):
        operation_res = self.rule(item)
        worry_lvl = operation_res % monkey_lcm

        if worry_lvl % self.test == 0:
            return self.if_true, worry_lvl
        return self.if_false, worry_lvl

    def round(self, monkey_lcm):
        throws = defaultdict(list)
        for item in self.list:
            monkey_idx, worry_lvl = self.__throw(item, monkey_lcm)
...
for i in range(10000):
    for j, monkey in enumerate(monkeys):
        ...
        throws = monkey.round(monkey_lcm)
