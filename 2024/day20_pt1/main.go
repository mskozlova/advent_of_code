...
func main() {
	file_name := "2024/day20_pt1/input.txt"
...
func findAllCheats(field []string, pos coord) map[coord]bool {
	cheats := make(map[coord]bool)
	queue := []step{{pos, 0}}
	...
	for len(queue) > 0 {
		...
		for _, d := range directions {
			...
			if (n == 0) && (string(field[next_c.row][next_c.col]) != "#") {
				continue
			}
			if (n == 1) && (string(field[next_c.row][next_c.col]) == "#") {
				continue
			}
	...
	for row := range len(field) {
		for col := range len(field[0]) {
			...
			for cheat := range cheats {
				...
				total_steps := steps_to_here + 2 + steps_to_finish
