...
def parse_trench_commands(file_name):
    ...
    with open(file_name, "r") as file:
        for line in file.readlines():
            direction, duration, color = line.strip().split()
            duration = int(duration)
            color = color[1:-1]
            commands.append((direction, duration, color))
    return commands


def build_trench(commands):
    field_size = len(commands)
    field = [[] for _ in range(2 * field_size + 1)]

    row, col = field_size + 1, field_size + 1

    for i, (direction, duration, _) in enumerate(commands):
